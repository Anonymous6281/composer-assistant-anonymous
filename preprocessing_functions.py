...
def load_and_clean_midisongbymeasure_from_midi_path(p):
    ...
    S = ms.MidiSongByMeasure.from_MidiSong(S, consume_calling_song=True)
    S.quantize_notes_by_measure(q=constants.QUANTIZE)
    S.change_cpq(to_cpq=ms.extended_lcm(constants.QUANTIZE))
...
def preprocess_midi_to_save_dict(p):
    """p a path"""
    S = load_and_clean_midisongbymeasure_from_midi_path(p)
